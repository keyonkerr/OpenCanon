...
| 要做的 | 写这里 |
|--------|--------|
| `opencanon/atoms/`、`opencanon/docs/`、`opencanon/config.yaml` 的路径 | 只改 `layout.rs` |
...
读路径在目录不存在时当作空或不存在，不创建目录。第一次成功写入原子才创建 `opencanon/atoms/`（必要时先建 `opencanon/`）。第一次成功写入派生文档才创建 `opencanon/docs/`。`add` 不建 `docs/`；`compose` 不建 `atoms/`；`init` 建 `opencanon/atoms/` 与 `config.yaml`。`skills/` 不写进使用方 `opencanon/`（`init` 把产品 skill 按同名覆盖复制到 `.agents/skills/`）。
