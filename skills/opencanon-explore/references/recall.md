...
本文件写抽词与调用 `query`。止于命中：不调 `freshness`，不 `edit`。种子、是否 `--all`、命中之后由本 skill 的 `SKILL.md` 本步给出。explore 的 SKILL 本步要求加 `--all`。
...
- 再为 `locales` 里每种语言补读者会用来提问的中心词；英语中心词可能只写在 tags 里。yaml 里已有 `en` 时与默认重复即可，不要去掉英语
