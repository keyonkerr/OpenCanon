...
按 [references/judge.md](references/judge.md) 的「无路径」节。本步不写盘。

将要新建、或 `same` 且命中为 draft、且没有 `impl-path`：一律 `ask`。`same` + active + 无路径 + 无新细节：不动。`same` + 无路径 + 有新细节：`ask_edit`。

完成：每个无路径且将要新建或复用 draft 的候选是 `ask`；每个无路径且有新细节的 `same` 是 `ask_edit`。

## 6. 提问

用 agent 的提问工具。一次表单可以放多道题。全部相关题目都有选择之前，不组 stdin，不调 `add` / `edit` / `active`。没有判定为 `true` 的（LLM 或人），不转正。步骤 4 已是 `true` / `false` / `auto_edit` / 不动的，不再问。

**是否同一事实**只问 `unsure`。题干列出拿不准的 hit（`id`、`title`、`body`）。选项：每条 hit 一个 `same_<id>`，外加 `different`。已是 `same` / `different` 的不再问。用户选 `different` 的改标为新建，沿用步骤 4 的真实性标号；若为 `ask` 或无路径，再问真实性。用户选 `same_<id>` 的沿用步骤 4 对该 hit 的标号；若尚未核且带 `impl-path`，先按 judge.md 核，`ask_edit` 补进本表单或紧接着再问。
...
**是否写入现有原子**只问 `ask_edit`。选项：`edit` 写进现有原子 / `keep` 现有正文不动。`edit` 的 `body` 是通写后的整篇主张：把现有原子没有的、且与代码一致的细节写进去后重写全文。

同一事实题、真实性题、`ask_edit` 可以放在同一表单。零题则直接写入。

完成：每个 `ask` 有且仅有一个真实性选项；每条曾为 `unsure` 的候选有且仅有一个同一事实选项；每个 `ask_edit` 有 `edit` 或 `keep`。

## 7. 写入

`add` 数组只含要新建的条目，下标只与这批新建对齐。

新建且 `false`：不创建。过滤后既无新建也无 `edit` / `auto_edit`：跳过本步写入，进入步骤 8。
...
2. **同一事实**且占用方是 active 或 draft：不 `add` 该条；按步骤 4/5/6 的 `auto_edit` / `edit` / `keep` / 不动；draft 且真实性为 `true` 则对该 id `active`
...
完成：本批该 `add` / `edit` / `active` 的已调用。进入步骤 8。
