...
已有命令、只要新流程：只加 `skills/<name>/SKILL.md`（需要时再加 `references/`），不动任何 crate。`add` / `edit` 的 stdin 模板写在对应 skill 里，那是调用面，不是领域字段表。

产品 skill 的 `name` 用 `opencanon-` 前缀，后面跟能力的英文（如 `opencanon-atomize`）。

## 代码读不出来的卡点

- 源文档在命令面之外：agent 自读全文；opencanon 不读、不写源文件。
- `opencanon/atoms/` 只经命令写入。skill 把结构化 JSON 交给 `add` / `edit`，不让 agent 拼 frontmatter 或直接改 md。
