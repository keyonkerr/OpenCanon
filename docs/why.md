...
- 要一篇派生可读文档时，按问题召回相关原子，由 LLM 整理成文：可调语序、写摘要，不得改变真源语义、不得引入原子里没有的事实。正文只写散文；工具校验所用原子均为 active，按 `atoms` 在文末生成依据表，写入派生目录。派生文不是真源；别处若要出现该文，只放链接。无需人审，因为不改真源。这是组合成文，不是去重合并，也不是按主题做确定性拼接。（[真源：拼接：要派生可读文档时按问题召回原子并由 LLM 组合成文，派生文不是真源](../opencanon/atoms/compose_from_atoms.md)）
- 问库里有什么、主张还成不成立、或结合代码看项目现状时，走只读探索：`query --all` 召回全状态原子，对照 `impl-path`，用结论回答该问，依据分三层列出，不落盘、不改真源。非 active 或低分标明后由人决定是否采信。（[真源：探索：按问题 query --all 对照代码作答且不写盘](../opencanon/atoms/explore_read_only.md)）
