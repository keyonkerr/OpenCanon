...
- 要一篇派生可读文档时，按问题召回相关原子，由 LLM 整理成文：可调语序、写摘要，不得改变真源语义、不得引入原子里没有的事实。工具只校验引用闭合并写入派生目录。派生文不是真源；别处若要出现该文，只放链接。无需人审，因为不改真源。这是组合成文，不是去重合并，也不是按主题做确定性拼接。（[真源：拼接：要派生可读文档时按问题召回原子并由 LLM 组合成文，派生文不是真源](../opencanon/atoms/compose_from_atoms.md)）
